...
class WoodokuEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        """
        output = np.zeros((15,15)).astype(np.float32)
        output[:9,3:12] = self._board
        output[10:,:5] = self._block_1
        output[10:,5:10] = self._block_2
        output[10:,10:] = self._block_3

        #output = cv2.resize(output, (160, 160), interpolation=cv2.INTER_LINEAR)
